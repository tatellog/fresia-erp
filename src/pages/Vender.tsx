...
export default function Vender() {
  ...
  return (
    <div className="pt-2">
      ...
      {done !== null && (
        <div className="fixed inset-x-4 top-16 z-50 mx-auto max-w-lg rounded-2xl bg-green-600 px-4 py-3 text-center font-bold text-white shadow-lg">
          ✓ Venta registrada · {money(done)}
        </div>
      )}

      {count > 0 && (
        <div className="fixed inset-x-0 bottom-[4.25rem] z-40 mx-auto max-w-lg px-4 pb-2">
        ...
        <div className="mb-4 space-y-2">
          {lines.map(l => (
            <div key={l.product.id} className="flex items-center justify-between gap-2">
              ...
              <div className="w-20 text-right font-bold tabular-nums">{money(l.product.price * l.qty)}</div>
            </div>
          ))}
        </div>
